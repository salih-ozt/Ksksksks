...
const HONEYPOT_PATHS = [
    ...
    // '/console' → KALDIRILDI: Android WebView / Chrome DevTools bu path'i kullanıyor
    //              Mobil uygulamamız bu yüzden ban yiyordu
    '/debug', '/_profiler', '/telescope',
...
// Mobil WebView / kendi uygulamamız için muaf UA listesi
const HONEYPOT_EXEMPT_UA = [
    'agrosocyal',           // Bizim Android app UA
    'agrolink',
    'wv)',                  // Android WebView işareti
    'webview',
    'dalvik',               // Android HTTP istemcisi
];

app.use((req, res, next) => {
    const path = req.path.toLowerCase();
    if (HONEYPOT_PATHS.some(hp => path === hp || path.startsWith(hp + '/'))) {
        // Kendi mobil uygulamamızı muaf tut
        const ua = (req.headers['user-agent'] || '').toLowerCase();
        const isOwnApp = HONEYPOT_EXEMPT_UA.some(exempt => ua.includes(exempt));
        if (isOwnApp) return next();
